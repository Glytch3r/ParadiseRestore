...
local H = ParadiseDev.Zones.Harness
local MODULE = "PZZoneHarness"
local RESULT = "result"
local ADMIN_STATE = "adminState"
local zoneBackup = nil

function H.copyTable(value, seen)
    if type(value) ~= "table" then return value end
    seen = seen or {}
    if seen[value] then return seen[value] end
    local result = {}
    seen[value] = result
    for key, item in pairs(value) do result[H.copyTable(key, seen)] = H.copyTable(item, seen) end
    return result
end
...
function H.uniqueZoneId(E, name)
...
function H.editorOptions(args)
    ...
    return {
        ...
        policy = {
            denyTags = H.parseTags(args and args.denyTags),
            requireAnyTags = H.parseTags(args and args.requireTags),
            adminBypass = not (args and args.adminBypass == false),
        },
    }
end

function H.validRegionArgs(args)
...
function H.adminState(E, player)
    local zones = {}
    for _, zone in pairs(E.zones) do
        ...
        zones[#zones + 1] = {
            ...
            denyTags = H.tagsText(zone.policy.denyTags),
            requireTags = H.tagsText(zone.policy.requireAnyTags),
...
function H.publish(E, player, message)
    E.syncAllBoundaryStates()
    H.adminState(E, player)
    if message then H.reply(player, message) end
end

function H.findOnlinePlayer(username, fallback)
    username = H.trim(username)
...
function H.nearestFeatureZone(E, target, feature)
...
function H.moveTargetToRegion(E, target, zone, region)
...
function H.featureIsValid(E, feature)
...
Events.OnClientCommand.Add(function(module, command, player, args)
    ...
    if not player or player:getAccessLevel() ~= "admin" then
        if player then H.reply(player, "Admin access is required for zone-harness commands.") end
        return
    end
    ...
    elseif command == "syncZones" then
        H.publish(E, player, "Zone state re-broadcast to all players.")
    elseif command == "demoHere" then
        local ok, message = H.createDemo(player)
        if ok then H.publish(E, player, message) else H.reply(player, message) end
    elseif command == "clear" then
        E.clear()
        H.publish(E, player, "All test zones and movement state cleared.")
    elseif command == "profile" then
        local target = args and args.username or player:getUsername()
        local profile = args and args.profile or "none"
        E.setProfile(target, H.tagsFor(profile))
        H.publish(E, player, "Profile for " .. tostring(target) .. " set to " .. tostring(profile) .. ".")
    elseif command == "vehicleMode" then
        E.vehicleMode = (args and args.mode == "rebound") and "rebound" or "observe"
        H.publish(E, player, "Vehicle mode: " .. E.vehicleMode)
    elseif command == "testFeature" then
        local feature = tostring(args and args.feature or "")
        if not H.featureIsValid(E, feature) then H.reply(player, "Select a valid zone feature.") return end
        local target = H.findOnlinePlayer(args and args.username, player)
        if not target then H.reply(player, "Target player is not online.") return end
        local zone, region = H.nearestFeatureZone(E, target, feature)
        if not zone then H.reply(player, "No zone currently has " .. feature .. " enabled.") return end
        H.moveTargetToRegion(E, target, zone, region)
        H.reply(player, "Moved " .. target:getUsername() .. " to nearest " .. feature .. " zone: " .. zone.name .. ".")
    elseif command == "probeFeature" then
        ...
        if not zone then H.reply(player, target:getUsername() .. " is not in a zone.") return end
        local flags = {}
        for _, key in ipairs(E.FEATURE_KEYS) do
            if zone.features and zone.features[key] then flags[#flags + 1] = key end
        end
        H.reply(player, target:getUsername() .. " authority: " .. zone.name .. " [" .. table.concat(flags, ", ") .. "]")
    elseif command == "cagePlayer" then
        local target = H.findOnlinePlayer(args and args.username, player)
        if not target then H.reply(player, "Target player is not online.") return end
        local zone = H.nearestFeatureZone(E, target, "isCage")
        if not zone then H.reply(player, "No Cage zone exists.") return end
        local ok, cageError = E.assignCage(target, zone)
        if not ok then H.reply(player, tostring(cageError)) return end
        H.reply(player, "Caged " .. target:getUsername() .. " in nearest Cage zone: " .. zone.name .. ".")
    elseif command == "uncagePlayer" then
        local target = H.findOnlinePlayer(args and args.username, player)
        if not target then H.reply(player, "Target player is not online.") return end
        if E.releaseCage(target) then
            H.reply(player, "Released " .. target:getUsername() .. " from Cage assignment.")
        else
            H.reply(player, target:getUsername() .. " is not caged.")
        end
    elseif command == "quickCreateZone" then
        local name = H.trim(args and args.name)
        if name == "" then name = "New Zone" end
        local px, py = math.floor(player:getX()), math.floor(player:getY())
        local region, regionError = H.validRegionArgs({
        ...
        if not region then H.reply(player, regionError) return end
        local id = H.uniqueZoneId(E, name)
        ...
        if not zone then H.reply(player, tostring(addError)) return end
        H.publish(E, player, "Created zone " .. name .. ". Select it and click feature icons.")
    elseif command == "toggleFeature" then
        ...
        if not ok then H.reply(player, tostring(featureError)) return end
        H.publish(E, player, tostring(E.zones[id].name) .. ": " .. key .. "=" .. tostring(enabled))
    elseif command == "setPrimaryPoint" then
        local id = tostring(args and args.id or "")
        local zone = E.zones[id]
        local region = zone and zone.regions[1] or nil
        if not region then H.reply(player, "The selected zone has no primary segment.") return end
        ...
        if not ok then H.reply(player, tostring(pointError)) return end
        H.publish(E, player, "Updated primary point " .. tostring(args and args.corner or 2) .. " for " .. zone.name .. ".")
    elseif command == "teleportToZone" then
        ...
        H.reply(player, "Teleported to " .. zone.name .. ".")
    elseif command == "backupZones" then
        zoneBackup = H.copyTable(E.zones)
        H.reply(player, "Zone backup captured in server memory.")
    elseif command == "restoreZones" then
        if not zoneBackup then H.reply(player, "No in-memory zone backup exists yet.") return end
        E.zones = H.copyTable(zoneBackup)
        E.lastValid = {}
        E.cageAssignments = {}
        E.rebuildIndex()
        H.publish(E, player, "Restored the in-memory zone backup.")
    elseif command == "createZone" then
        local id = tostring(args and args.id or "")
        if id == "" then H.reply(player, "Zone ID is required.") return end
        if E.zones[id] then H.reply(player, "Zone ID already exists: " .. id) return end
        local region, regionError = H.validRegionArgs(args)
        if not region then H.reply(player, regionError) return end
        local options, optionError = H.editorOptions(args)
        if not options then H.reply(player, optionError) return end
        local zone, addError = E.addRegion(id, region.x1, region.y1, region.x2, region.y2, options)
        if not zone then H.reply(player, tostring(addError)) return end
        H.publish(E, player, "Created zone " .. id .. ".")
    elseif command == "updateZone" then
        local id = tostring(args and args.id or "")
        local options, optionError = H.editorOptions(args)
        if not options then H.reply(player, optionError) return end
        local ok, updateError = E.updateZone(id, options)
        if not ok then H.reply(player, tostring(updateError)) return end
        H.publish(E, player, "Updated zone " .. id .. ".")
    elseif command == "addSegment" then
        local id = tostring(args and args.id or "")
        if not E.zones[id] then H.reply(player, "Select an existing zone first.") return end
        local region, regionError = H.validRegionArgs(args)
        if not region then H.reply(player, regionError) return end
        local zone, addError = E.addRegion(id, region.x1, region.y1, region.x2, region.y2, {})
        if not zone then H.reply(player, tostring(addError)) return end
        H.publish(E, player, "Added a segment to " .. id .. ".")
    elseif command == "updateSegment" then
        ...
        if not ok then H.reply(player, tostring(updateError)) return end
        H.publish(E, player, "Updated the selected segment in " .. id .. ".")
    elseif command == "removeSegment" then
        local id = tostring(args and args.id or "")
        local ok, removeError = E.removeRegion(id, args and args.regionIndex)
        if not ok then H.reply(player, tostring(removeError)) return end
        H.publish(E, player, "Removed the selected segment from " .. id .. ".")
    elseif command == "deleteZone" then
        local id = tostring(args and args.id or "")
        local ok, removeError = E.removeZone(id)
        if not ok then H.reply(player, tostring(removeError)) return end
        H.publish(E, player, "Deleted zone " .. id .. ".")
    elseif command == "status" then
        local count = 0
        for _ in pairs(E.zones) do count = count + 1 end
        H.reply(player, "Zones=" .. count .. " vehicleMode=" .. E.vehicleMode .. " events=" .. #E.eventLog)
    end
end)
