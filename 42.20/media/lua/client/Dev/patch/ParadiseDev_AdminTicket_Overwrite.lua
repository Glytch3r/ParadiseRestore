...
function ISAdminTicketsUI:drawDatas(y, item, alt)
    ...
    local authorName = ticket:getAuthor()
    local isOnline = getPlayerFromUsername(authorName) ~= nil
    
    if isOnline then
        self:drawRect(UI_BORDER_SPACING, y + (FONT_HGT_SMALL / 2), 6, 6, 1, 0.2, 0.8, 0.2)
    else
        self:drawRect(UI_BORDER_SPACING, y + (FONT_HGT_SMALL / 2), 6, 6, 1, 0.6, 0.6, 0.6)
    end

    self:drawText(authorName, UI_BORDER_SPACING + 10, y + 3, 1, 1, 1, a, self.font);
